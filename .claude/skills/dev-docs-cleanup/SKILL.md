...
Two harnesses share this repo. `CLAUDE.md` + `.claude/skills/` are the
**authority**; `AGENTS.md` + `.agents/skills/` are generated adapters (`R7`,
and the Authority line at the top of `CLAUDE.md` says so literally in both
...
*improvement* found in an adapter is merged into `CLAUDE.md` /
...
`CLAUDE.md` is **tracked** here. A session that may not commit edits **neither**
