...
3. **Doctrine ahead → read `../../Rust/doctrine/CHANGELOG.md` forward from the
   ...
   - **`[skills-update]`** — merge into this repo's **declared authority**
     (`CLAUDE.md` + `.claude/skills/`) and regenerate the adapters in the same
     ...
     adapter. `CLAUDE.md` is tracked here, so a session that may not commit
...
- **Know which of the four files a source change touches.** Adding or changing
  ...
  that has missed the fragment framework (CLAUDE.md).
...
CLAUDE.md — release-mode wheel, the statistic the harness records per cell,
