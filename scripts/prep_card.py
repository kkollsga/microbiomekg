...
def obo_parents(terms: dict, term_id: str) -> list[str]:
    ...
    return [line.split("!")[0].strip() for line in terms.get(term_id, {}).get("is_a", [])]
...
def categories(model: dict, class_name: str) -> list[tuple[str, str]]:
    ...
    for entry in (model.get("ARO_category") or {}).values():
        if entry.get("category_aro_class_name") == class_name:
            out.append((aro_id(entry.get("category_aro_accession")),
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory holding card-data/ and card-ontology/ "
        "(default: <raw>/card).",
    )
    ...
    missing = [p for p in (models_path, index_path, pmid_path, obo_path) if not p.is_file()]
    if missing:
    ...
    genes = Writer(
        ...
        ["aro_id", "name", "card_short_name", "description", "gene_family",
         "gene_family_aro", "model_type", "card_model_id", "curated",
         "protein_accession", "dna_accession", "aro_parents", "name_source",
         "publications", "n_publications", "source", "source_licence"],
        key="aro_id", merge=True, owner=("source", SOURCE),
    )
    drug_classes = Writer(
        out / "drug_class.csv",
        ["drug_class_id", "label", "aro_parents", "source", "source_licence"],
        key="drug_class_id", merge=True, owner=("source", SOURCE),
    )
    mechanisms = Writer(
        out / "resistance_mechanism.csv",
        ["mechanism_id", "label", "aro_parents", "source", "source_licence"],
        key="mechanism_id", merge=True, owner=("source", SOURCE),
    )
    confers = Writer(
    ...
    for model_id in sorted(set(index_models) - set(models), key=_as_int):
        ...
        disagreements.add({
            "model_id": model_id, "aro_id": index_models[model_id],
            "kind": "absent-from-card.json",
            "card_json": "", "aro_index": index_models[model_id], "source": SOURCE,
        })
    for model_id in sorted(set(models) - set(index_models), key=_as_int):
        counters["json_only"] += 1
        disagreements.add({
            "model_id": model_id, "aro_id": aro_id(models[model_id]["ARO_accession"]),
            "kind": "absent-from-aro_index.tsv",
            "card_json": aro_id(models[model_id]["ARO_accession"]),
            "aro_index": "", "source": SOURCE,
        })
    ...
    for model_id in sorted(models, key=_as_int):
        ...
        if ontology_name and card_name and ontology_name != card_name:
            ...
            disagreements.add({
                "model_id": model_id, "aro_id": term, "kind": "name-differs",
                "card_json": card_name, "aro_index": ontology_name, "source": SOURCE,
            })
        ...
        for accession, label in categories(model, DRUG_CLASS):
            ...
            also = _stated_in_obo(terms, term, "confers_resistance_to_drug_class",
                                  accession)
            confers.add({
                "aro_id": term, "drug_class_id": accession, **edge,
                "source_licence": card.licence_for(also),
                "source_relation": (
                    "aro.obo:confers_resistance_to_drug_class" if also
        ...
        for accession, label in categories(model, MECHANISM):
            ...
            via.add({
                "aro_id": term, "mechanism_id": accession, **edge,
        ...
        if not curated:
            ...
            unresolved_assoc.add({
                ...
                "reported_name": name, "reported_tax_id": "",
        ...
        if resolution.tax_id is None:
            ...
            unresolved_assoc.add({
                "source_record_id": edge["source_record_id"],
                "reported_name": reported_name, "reported_tax_id": raw_id,
    ...
    print(f"\nread {counters['models']:,} models from card.json "
          f"(CARD {version})")
    print(f"  edges: {counters['confers_resistance_to']:,} CONFERS_RESISTANCE_TO, "
          f"{counters['via_mechanism']:,} VIA_MECHANISM, "
          f"{counters['carries_resistance_gene']:,} CARRIES_RESISTANCE_GENE")
    print(f"  not loaded: {counters['without_reference_sequence']:,} meta-models with "
          f"no reference sequence, {counters['unresolved_taxa']:,} unresolved taxa")
    print(f"  disagreements with aro_index.tsv: {counters['index_only']:,} models the "
          f"index has and card.json does not, {counters['json_only']:,} the other way, "
          f"{counters['name_disagreement']:,} name differences")
    print(f"  citations: {counters['models_with_citation']:,} models with at least one "
    ...
    print("  reference-sequence taxa: " + ", ".join(
        f"{k} {n:,}" for k, n in sorted(specificity.items())))
