...
def main(argv: list[str] | None = None) -> int:
    ...
    malformed = sorted(
        k
        for k in metabolites
        if not COMPOUND_ID.match(k) and not k.startswith(("D", "G"))
    )
    withdrawn = sorted(
        k for k in metabolites if COMPOUND_ID.match(k) and k not in compounds
    )
    other_namespace = sorted(k for k in metabolites if k.startswith(("D", "G")))

    for pathway_raw, compound_raw in (
        (r[0], r[1]) for r in rows(src / "link_compound_pathway.tsv", 2)
    ):
    ...
    print(
        f"  not loaded: {counters['link_without_metabolite']:,} links whose compound "
        f"is not a selected metabolite, {counters['link_without_pathway']:,} links "
        f"naming an undeclared map"
    )
    print(
        f"  {len(withdrawn):,} of the {len(metabolites):,} KEGG ids HMDB carries are "
    ...
    print(
        f"  {len(malformed):,} are not a KEGG identifier at all"
        + (": " + ", ".join(malformed) if malformed else "")
    )
    print(
        f"  {len(other_namespace):,} are not compound ids at all (D = drug, "
        f"G = glycan) and can never join a compound list"
        + (": " + ", ".join(other_namespace[:8]) if other_namespace else "")
    )
    print(
        "  conv_compound_pubchem.tsv NOT loaded: those are PubChem Substance "
